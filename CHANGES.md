...
- Add `autoSync` opt-in for transparent local persistence:
  ...
  - **Auto-hydrate**: the model/collection loads its stored state on construction. `await model.initialized` resolves when done (resolves immediately when `autoSync` is off, so callers can always `await` it uniformly).
  - **Auto-save**: any `set()` (including `attrs.x = y`) that produces changes schedules a debounced save. Pass `{ noAutoSave: true }` to opt out per-call.
